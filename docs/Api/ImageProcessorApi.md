...
| [**imageProcessorImportImageFromUrl()**](ImageProcessorApi.md#imageProcessorImportImageFromUrl) | **POST** /api/processor/v1/images/import/from-url | Imports image from remote source file and saves it to storage. |
...
## `imageProcessorImportImageFromUrl()`

```php
imageProcessorImportImageFromUrl($tenant_id, $import_image_from_url_model): \Aurigma\AssetProcessor\Model\ImageDto
```
...
try {
    $result = $apiInstance->imageProcessorImportImageFromUrl($tenant_id, $import_image_from_url_model);
    print_r($result);
} catch (Exception $e) {
    echo 'Exception when calling ImageProcessorApi->imageProcessorImportImageFromUrl: ', $e->getMessage(), PHP_EOL;
}
```
